...
- (void)addInfiniteScrollingWithActionHandler:(void (^)(void))actionHandler {
    ...
    if(!self.infiniteScrollingView) {
        ...
        //easeeeeeeeee add
        [view setCustomView:[view viewForStateLoading] forState:SVInfiniteScrollingStateLoading];
...
- (UIView *)viewForStateLoading{
    ...
    [activityIndicator setCenter:CGPointMake(kScreen_Width/2-60, CGRectGetHeight(customView.frame)/2)];
    
    //提示文字
    UILabel *loadingLabel = [[UILabel alloc] initWithFrame:CGRectMake(kScreen_Width/2-30, (CGRectGetHeight(customView.frame)-25)/2, 100, 25)];
    loadingLabel.textColor = [UIColor lightGrayColor];
    loadingLabel.font = [UIFont systemFontOfSize:16];
    loadingLabel.textAlignment = NSTextAlignmentCenter;
    loadingLabel.text = @"正在加载中…";
